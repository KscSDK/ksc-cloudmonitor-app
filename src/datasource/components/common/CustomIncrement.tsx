...
const CustomIncrement: FC<OwnProps> = ({
  datasource,
  onAdd,
  onDelete,
  onValueChange,
  ...
  const deleteItem = (delIndex: number) => {
    ...
    const delArr = delArrays.filter(
      (item: string, index: number) => index !== delIndex
    );
    setIncrement(delArr);
    ...
    <>
      ...
      {incrementData.map((item: any, index: number) => (
        <>
          <CustomInput
            defaultValue={item}
            onChange={(value: string) => dealValueByIndex(value, index)}
          />
          <IconButton
