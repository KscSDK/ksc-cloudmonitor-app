...
export const getSignKs3 = async (
  pluginId: number,
  proxyKey: string,
  { extenQuery = '', region = 'cn-beijing', method = 'GET', postParams = {} },
  timestamp: number
) => {
  const proxyConfig: any = ServiceMap.get(proxyKey);
...
export const requestKs3 = async (instanceSetting: any, proxyKey: string, queryParams: any) => {
  const { id: pluginId, url } = instanceSetting;
  const { extenQuery, region, method = 'GET', postParams = undefined } = queryParams;
  const utcTime = moment().utc();
  const timestamp = utcTime.unix();
  // 获取签名
  const sign = await getSignKs3(pluginId, proxyKey, { region, extenQuery, method, postParams }, timestamp);
  let serviceKey = proxyKey;
  if (sign.data.intranet) {
    serviceKey += '-internal';
  }
